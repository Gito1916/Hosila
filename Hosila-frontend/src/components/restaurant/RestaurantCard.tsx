...
export function RestaurantCard({ service, onOrder, cartQuantity = 0 }: RestaurantCardProps) {
    ...
    return (
        <button
            ...
            className={`w-full text-left bg-surface-card border rounded-lg p-3 transition-all relative ${isUnavailable
                    ? 'border-border opacity-60 cursor-not-allowed'
                    : isInCart
                        ? 'border-status-available ring-1 ring-status-available/50'
                        : 'border-border hover:border-primary-500 hover:bg-surface-raised active:scale-[0.98]'
                }`}
        >
            {/* Quantity Badge */}
            {isInCart && (
                <div className="absolute -top-2 -right-2 w-6 h-6 bg-status-available rounded-full flex items-center justify-center text-heading text-xs font-bold shadow-lg">
                    {cartQuantity}
                </div>
            )}
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                        <h3 className="text-heading font-medium text-sm truncate">{service.name}</h3>
                        {isUnavailable && (
                ...
                <div className="text-right shrink-0">
                    <p className="text-sm font-bold text-heading">₦{service.price.toLocaleString()}</p>
                    {isInCart && (
